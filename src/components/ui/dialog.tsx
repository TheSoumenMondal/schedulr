...
function DialogClose({ className, children, disabled }: DialogCloseProps) {
  ...
  return (
    <button
      ...
      className={cn(
        "absolute top-4 right-4 rounded-xs opacity-70 transition-opacity cursor-pointer",
      ...
      disabled={disabled}
    >
      {children || <IconX className="h-4 w-4 hover:-rotate-90 transition-transform duration-300"/>}
